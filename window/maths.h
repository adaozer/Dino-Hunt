...
public:
	...
	static float Dot(const Vec3& v1, const Vec3& pVec) {
		return v1.v[0] * pVec.v[0] + v1.v[1] * pVec.v[1] + v1.v[2] * pVec.v[2];
	}
...
public:
	...
	static Matrix rotateX(float angle) {
		...
		m.m[5] = cos; m.m[6] = -sin; m.m[9] = sin; m.m[10] = cos;
		return m;
	}
	...
	static Matrix projMatrix(float aspect, float fov, float f, float n) {
		Matrix m;
	...
	static Matrix lookAt(const Vec3& from, const Vec3& to, const Vec3& up) {
		...
		Matrix m;
		m.m[0] = right.x; m.m[1] = right.y; m.m[2] = right.z; m.m[3] = Vec3::Dot(-from, right);
		m.m[4] = upPrime.x; m.m[5] = upPrime.y; m.m[6] = upPrime.z; m.m[7] = Vec3::Dot(-from, upPrime);
		m.m[8] = dir.x; m.m[9] = dir.y; m.m[10] = dir.z; m.m[11] = Vec3::Dot(-from, dir);
		m.a[3][3] = 1;
		return m;
	}
...
public:
	...
	static Quaternion slerp(Quaternion q1, Quaternion q2, float t) {
		...
		Quaternion q22 = dot < 0 ? -q2 : q2;
		dot = dot > 0 ? dot : -dot;

		float theta = acosf(std::clamp(dot, -1.0f, 1.0f));
		...
		Quaternion qr(q1.a * first + q22.a * second, q1.b * first + q22.b * second, q1.c * first + q22.c * second, q1.d * first + q22.d * second);
		qr = qr.normalise();
		return qr;
	}
